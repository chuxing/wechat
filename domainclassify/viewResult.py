...
for label in labels:
    ...
    if realtotal != 0:
        ...
        rt += r * (realtotal * 1.0 / len(realLabels)) 
    if predtotal != 0:
        p = confuseMatrix[label][label] / predtotal
        pt += p * (realtotal * 1.0 / len(realLabels))
    if p != 0 or r != 0:
        f = (p * r * 2) / (p + r)
        ft += f * (realtotal * 1.0 / len(realLabels))
...
#print "Total Correct: %d/%d=%f  Total prediction: %f  Total Recall:%f  Total F1:%f" % (totalCorrec, len(realLabels), totalCorrec/len(realLabels), sum(ps)/len(ps), sum(rs)/len(rs), sum(fs)/len(fs))
print "Total Correct: %d/%d=%f  Total predict: %f  Total Recall:%f  Total F1:%f" % (totalCorrec, len(realLabels), totalCorrec/len(realLabels), pt, rt, ft) 
