...
sout_conflict = open(sys.argv[3], "w")
...
for pred, raw in zip(pred_data, raw_data):
    ...
    if pred_label != label:
        ...
        print >> sout_conflict, "R:" + label + "\tP:" + pred_label + "\t" + " ".join(raw.split(" ")[1:]).strip() + "\tP_dianbo : " + str(pred_weights[0]) + " P_normal: " + str(pred_weights[1]) 
